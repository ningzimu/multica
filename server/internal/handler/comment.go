...
func (h *Handler) DeleteComment(w http.ResponseWriter, r *http.Request) {
	...
	if deleted.Tombstone != nil {
		...
		eventPayload := deleted.webhookPayload(comment)
		eventPayload["comment"] = resp
		eventPayload["comment_deleted"] = true
	...
	for _, removedID := range deleted.RemovedIDs {
		eventPayload := deleted.webhookPayload(comment)
		eventPayload["comment_id"] = uuidToString(removedID)
		// Pruning an already-deleted ancestor is only realtime cache cleanup.
		eventPayload["tombstone_pruned"] = removedID != comment.ID
		if deleted.IssueRevision > 0 {
			eventPayload["issue_revision"] = deleted.IssueRevision
		}
...
// webhookPayload carries only identity and the event-time parent snapshot;
// deletion events never expose the deleted body.
func (d commentDeletion) webhookPayload(comment db.Comment) map[string]any {
	return map[string]any{
		"comment_id": uuidToString(comment.ID), "issue_id": uuidToString(comment.IssueID),
		"parent_id": uuidToPtr(comment.ParentID), "author_type": comment.AuthorType,
		"author_id": uuidToString(comment.AuthorID), "comment_type": comment.Type,
		"issue_title": d.IssueTitle, "issue_status": d.IssueStatus,
		"issue_priority": d.IssuePriority, "issue_project_id": uuidToPtr(d.IssueProjectID),
	}
}
...
func (h *Handler) deleteComment(ctx context.Context, commentID, workspaceID pgtype.UUID) (commentDeletion, error) {
	...
	out.IssueTitle = target.IssueTitle
	out.IssueStatus = target.IssueStatus
	out.IssuePriority = target.IssuePriority
	out.IssueProjectID = target.IssueProjectID
